...
- **量子化**: 拍グリッド方式(既定、ブラウザから選択可能)。テンポ変化がある曲向けの秒グリッド方式(`mode: "seconds"`)も用意されているが、ブラウザの変換設定パネルには公開しておらず現時点では API 直接呼び出し専用。量子化誤差の統計(最大/平均ms、移動ノート数)も返す
...
- **OMR(実験的機能)**: PDF・スキャン画像から homr (既定) または Audiveris で MusicXML を生成し、通常のパイプラインへ合流

## OMR(光学楽譜認識、PDF/画像対応)

PDF・画像のアップロードは既定では無効です。別コンテナを起動すると使えるようになります。既定のエンジンは
[homr](https://github.com/liebharc/homr)(ディープラーニングベース。グランド譜表=両手ピアノの認識に強い):

```bash
docker compose --profile homr up --build
```

Audiveris(古典的なOMR。単旋律譜等で homr より高速な場合がある)へ切り替えるには
`OMR_SERVICE_URL` を上書きしてこちらの profile を起動します:

```bash
OMR_SERVICE_URL=http://omr:8080 docker compose --profile omr up --build
```
...
- 各コンテナの詳細(HTTP API、対応形式、ビルド・検証方法、ライセンス)は [homr/README.md](homr/README.md)・[omr/README.md](omr/README.md) を参照してください
...
| `OMR_SERVICE_URL` | `http://homr:8080` | OMR コンテナの接続先(Audiveris に切り替えるには `http://omr:8080`) |
